#!/usr/bin/env node
import { intro, outro, text, select, multiselect, confirm, spinner, isCancel, } from "@clack/prompts";
...
async function generatePackageJson(config, projectDir) {
    const dependencies = {};
    ...
    const packageJson = {
        ...
        private: true,
        scripts: {
            dev: config.bundler === "vite"
                ? "vite"
                : config.bundler === "rspack"
                    ? "rspack serve"
                    : 'echo "No bundler configured"',
            build: config.bundler === "vite"
                ? "vite build"
                : config.bundler === "rspack"
                    ? "rspack build"
                    : 'echo "No bundler configured"',
...
async function generateHtmlTemplate(config, projectDir) {
...
<body>
  ...
  ${config.framework === "React"
    ...
    try {
        ...
        if (error.code === "EACCES") {
            throw new Error(`Permission denied: Cannot write to index.html. Try running with higher privileges.`);
        }
        throw error;
    }
}
// Function to generate framework specific files
async function generateFrameworkFiles(config, projectDir) {
    try {
        await mkdir(join(projectDir, "src"), { recursive: true });
    }
    catch (error) {
        if (error.code === "EACCES") {
            throw new Error(`Permission denied: Cannot create src directory. Try running with higher privileges.`);
        }
        throw error;
    }
    if (config.framework === "React") {
...
export default App;`;
        ...
        await writeFile(join(projectDir, "tsconfig.node.json"), JSON.stringify(tsconfigNodeContent, null, 2));
